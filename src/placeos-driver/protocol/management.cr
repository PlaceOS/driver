...
class PlaceOS::Driver::Protocol::Management
  ...
  def initialize(@driver_path : String, @on_edge : Bool = false)
    ...
    @sequence = 1_u64
    spawn(name: "process_events") { process_events }
  end
  ...
  private def shutdown(terminated = true) : Nil
    ...
    spawn(name: "ensure_shutdown") { ensure_shutdown(exited) }
  ...
  private def start_process : Nil
    ...
    io = begin
      spawn(name: "launch_driver") { launch_driver(unix_socket) }
    ...
    begin
      ...
      spawn(name: "process_comms") { process_comms(io, loaded) }
      loaded.get
