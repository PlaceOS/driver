...
class ACAEngine::Driver::TransportWebsocket < ACAEngine::Driver::Transport
  # timeouts in seconds
  def initialize(@queue : ACAEngine::Driver::Queue, @uri : String, @settings : ::ACAEngine::Driver::Settings, &@received : (Bytes, ACAEngine::Driver::Task?) -> Nil)
    @terminated = false
    @logger = @queue.logger
